...
def train_lfm(
    ...
    if best_epoch is not None:
        ...
        save_path = "/mnt/lustre/helios-home/gartmann/venv/checkpoints/late_fusion/lfm_{}.pt".format(
        ...
        df_train.to_csv(
            f"./diagnostics/metrics-df/lfm/df_train_{date_time}.csv"
        )
        df_val.to_csv(
            f"./diagnostics/metrics-df/lfm//df_val_{date_time}.csv"
        )
        df_test.to_csv(
            f"./diagnostics/metrics-df/lfm/df_test_{date_time}.csv"
        )
...
def train_one_epoch(
    ...
    # Save checkpoint
    save_path = f"/mnt/lustre/helios-home/gartmann/venv/checkpoints/{epoch_idx}_at{time.strftime('%Y-%m-%d_%H-%M')}_{seed}.pt"
    torch.save(
        {
            "model_state_dict": model.state_dict(),
            "optimizer_state_dict": optimizer.state_dict(),
...
def test_model(model, plane, checkpoint_pth, test_loader, device, df, test_table, seed):
    ...
    date_time = time.strftime("%d-%m_%H-%M")
    df.to_csv(
        f"./diagnostics/metrics-df/df_test_{date_time}_plane{plane}_.csv"
    )
...
def train_model(
    ...
    # Save checkpoint
    save_path = (
        "/mnt/lustre/helios-home/gartmann/venv/checkpoints/{}_{}_dset_{}.pt".format(
        seed, plane, dset_seed
        )
    )
    ...
    df_train.to_csv(
        f"./diagnostics/metrics-df/df_train_{date_time}_plane{plane}_{seed}.csv"
    )
    df_val.to_csv(
        f"./diagnostics/metrics-df/df_val_{date_time}_plane{plane}_{seed}.csv"
    )
